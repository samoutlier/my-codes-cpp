...
void Right_View_Traverse(Node *&root, int level, vector<int> &ans)
{
    if (!root)
        return;
    ...
    Right_View_Traverse(root->right, level + 1, ans);
    Right_View_Traverse(root->left, level + 1, ans);
}
...
int main()
{
    ...
    vector<int> ans;
    int level = 0;
    Right_View_Traverse(root, level, ans);
